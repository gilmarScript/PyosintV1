...
class Virustotal(enumratorBaseThreaded):
    ...
    def enumerate(self):
        if self.apikey:
            ...
            self.print_(R + " Erro: variável de ambiente chave da API do VirusTotal não encontrada. Ignorando" + W)
...
def main(domain, threads, savefile, ports,verbose,silent=None):
    try:
        ...
        if subdomains:
            ...
            if ports:
                if not silent:
                    print(G + " Inicie a varredura de porta agora para a seguinte portas: %s%s" % (Y, ports) + W)
                ports = ports.split(',')
        ...
        print("\n Oh merda Teclado pressionado Agora prepare-se para ver um monte de erro")
        time.sleep(5)
        exit(0)
